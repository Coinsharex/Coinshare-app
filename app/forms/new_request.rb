  ...
  module Form
    ...
    class NewRequest < Dry::Validation::Contract
      ...
      params do
        ...
        required(:description).filled(max_size?: 1440)
      ...
      rule(:title) do
        if value.length < 20 and value.length > 255
          key.failure('provide title with at least 20 characters and up to 225 characters')
        end
      end

      rule(:description) do
        if value.length < 200 and value.length > 1440
      ...
      rule(:amount) do
        key.failure('The entered value should be between 5$ and 5000$') unless value.to_i >= 5 and value.to_i <= 5000
      end
